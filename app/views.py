...
def insuranceHome(request):
    ...
    data = {
        "insurancePolicies": InsurancePolicySerializer(
            InsurancePolicy.select_related("company"), many=True
        ).data,
        "policyHolders": PolicyHolderSerializer(
            PolicyHolder.select_related(
                "agent", "policy", "branch", "company"),
            many=True,
        ).data,
        "claims": ClaimRequestSerializer(
            ClaimRequest.select_related("policy_holder", "company"), many=True
        ).data,
        "salesAgents": SalesAgentSerializer(
            SalesAgent.select_related("company"), many=True
        ).data,
        "branches": BranchSerializer(Branch, many=True).data,
    }
...
def createPolicy(request):
    ...
    if not request.user.is_superuser:
        # Automatically assign company for non-superusers
...
def policyDetails(request, id):
    if request.user.is_superuser:
        ...
        policy = get_object_or_404(
            InsurancePolicy.objects.filter(company=request.user.company), id=id
        )
...
def updatePolicy(request, id):
    policy = get_object_or_404(
        InsurancePolicy, id=id, company=request.user.company)
...
def getAllPolicyHolders(request):
    holders = PolicyHolder.objects.filter(company=request.user.company).select_related(
        "agent", "policy", "branch", "company"
    )
...
def policyHolderDetails(request, id):
    try:
        ...
        if holder.policy:
            ...
            _, loaded_annual_premium, interval_payment = (
            ...
            return Response(
                {"error": "PolicyHolder has no associated policy."}, status=400
            )
...
def updatePolicyHolder(request, id):
    holder = get_object_or_404(
        PolicyHolder, id=id, company=request.user.company)
    ...
    try:
        ...
        serializer = PolicyHolderSerializer(
            holder, data=request.data, partial=True)
        if serializer.is_valid():
...
def getAllClaims(request):
    claims = ClaimRequest.objects.filter(company=request.user.company).select_related(
        "policy_holder", "company"
    )
...
def claimDetails(request, id):
    if request.user.is_superuser:
        ...
        claim = get_object_or_404(
            ClaimRequest.objects.filter(company=request.user.company), id=id
        )
...
def agentDetails(request, id):
    if request.user.is_superuser:
        ...
        agent = get_object_or_404(
            SalesAgent.objects.filter(company=request.user.company), id=id
        )
...
def paymentDetails(request, id):
    if request.user.is_superuser:
        ...
        payment = get_object_or_404(
            PremiumPayment.objects.filter(company=request.user.company), id=id
        )
...
def premiumPaymentsDue(request, dueDate):
    payments = PremiumPayment.objects.filter(
        due_date__lte=dueDate, status="Due")
    return Response(PremiumPaymentSerializer(payments, many=True).data)
...
def underwritingDetails(request, id):
    if request.user.is_superuser:
        ...
        underwriting = get_object_or_404(
            Underwriting.objects.filter(company=request.user.company), id=id
        )
...
def updateUnderwriting(request, id):
    underwriting = get_object_or_404(Underwriting, id=id)
    serializer = UnderwritingSerializer(
        underwriting, data=request.data, partial=True)
    if serializer.is_valid():
        serializer.save()
        return Response(serializer.data)
    return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
def employeeDetails(request, id):
    if request.user.is_superuser:
        ...
        employee = get_object_or_404(
            Employee.objects.filter(company=request.user.company), id=id
        )
...
def companyDetails(request, id):
    ...
    if request.user.is_superuser:
        ...
        company = get_object_or_404(
            Company.objects.filter(id=id, user=request.user), id=id
        )
...
def markPaymentAsPaid(request, payment_id):
    ...
    payment.status = (
        "Paid" if payment.total_paid >= payment.annual_premium else "Partially Paid"
    )
    payment.save()
        ...
        return JsonResponse(
            {
                ...
                "policy": {
                    "fixed_premium_ratio": float(
                        policy_holder.policy.fixed_premium_ratio
                    ),
...
def manage_mortality_rates(request):
    ...
    if request.method == "POST":
        if "generate" in request.POST and generator_form.is_valid():
            ...
            bulk_form = MortalityRateBulkForm(
                request.POST, age_ranges=age_ranges)
            ...
            if bulk_form.is_valid():
                try:
                    ...
                    messages.success(
                        request, "Mortality rates have been updated successfully."
                    )
